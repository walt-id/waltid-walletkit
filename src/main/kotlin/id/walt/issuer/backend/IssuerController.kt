...
object IssuerController {
  ...
  fun requestIssuance(ctx: Context) {
    ...
    if(session != null) {
      ...
      val initiationRequest = IssuerManager.newIssuanceInitiationRequest(userInfo.id, selectedIssuables, isPreAuthorized, userPin)
  ...
  fun token(ctx: Context) {
    ...
    if(tokenReq.authorizationGrant.type == PreAuthorizedCodeGrant.GRANT_TYPE) {
      val pinMatches = session.userPin?.let { it == (tokenReq.authorizationGrant as PreAuthorizedCodeGrant).userPin } ?: true
      if(!pinMatches) {
