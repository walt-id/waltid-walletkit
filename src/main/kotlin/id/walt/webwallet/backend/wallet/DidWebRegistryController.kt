...
object DidWebRegistryController {
    ...
    fun registerDidWeb(ctx: Context) {
        ...
        val match = "did:web:${Regex.escape(domainDidPart)}:${Regex.escape(rootPathDidPart)}:([^:]+)".toRegex().matchEntire(did.id) ?: throw BadRequestResponse("did:web doesn't match this registry domain and path")
