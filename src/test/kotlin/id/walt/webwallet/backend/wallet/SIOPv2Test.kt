...
class SIOPv2Test : AnnotationSpec() {
    ...
    val testUserContext =
        UserContext(HKVKeyStoreService(), HKVVcStoreService(), FileSystemHKVStore(FilesystemStoreConfig("data/test@mail.com")))
    ...
    fun init() {
        ...
        siopv2RequestParams = siopv2Request.toUriQueryString().split("&").map { it.split("=") }
    ...
    fun testPresentationExchange() {
        ...
        every { ctx1.queryParam(not("subject_did")) } answers { siopv2RequestParams[firstArg()] }
        ...
        val idToken = Klaxon().parse<SIOPv2IDToken>(JWTParser.parse(peResponse.id_token).jwtClaimsSet.toString())
        val vpToken = Klaxon().parse<SIOPv2VPToken>(JWTParser.parse(peResponse.vp_token).jwtClaimsSet.toString())

        idToken!!.subject shouldBe subjectDid
        vpToken!!.vp_token shouldHaveSize 1

        val presentation = vpToken.vp_token.first().presentation.toCredential()

        presentation should beOfType<VerifiablePresentation>()

        with(presentation as VerifiablePresentation) {
            verifiableCredential shouldHaveSize 1
            verifiableCredential.first().id shouldBe selectedCC.credentialId
        }
    }
}
